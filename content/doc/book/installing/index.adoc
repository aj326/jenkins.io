...
Jenkins is available in the fedora repository, so you can install it using the `dnf` package manager.
...
This package installation will install all the dependencies to get Jenkins set up.

You can run the Jenkins service with `systemd`.
...
The line above will immediately start the Jenkins service and will start the Jenkins service on subsequent boots 
